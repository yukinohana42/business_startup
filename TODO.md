...
今回、Day1当日に子供たちへ話すためのgamified v2レクチャーprompt packetを `slides/kids/day1_gamified_v2_lecture_prompt_packet.md` に追加した。
これは8枚構成で、1枚1メッセージ、価値変化カード連動、各スライドのデザイン、画像生成promptをまとめたもの。
さらに、Day2当日に子供たちへ話すためのgamified v2レクチャーprompt packetを `slides/kids/day2_gamified_v2_lecture_prompt_packet.md` に追加した。
こちらは8枚構成で、1枚1メッセージ、価値の種カード連動、各スライドのデザイン、画像生成promptをまとめたもの。
今回、Day2の具体例として、地域で働く若手医師へのオンライン指導事業を1枚で見せるinfographic promptを `infographics/day2_regional_doctor_online_mentoring_case_prompt.md` に追加した。

次は、Day1とDay2の当日レクチャー用8枚構成を人間が確認する。
確認後、Day1とDay2のスライド画像を1枚ずつ生成して候補に置く。
あわせて、gamified v2全体と親向けgamified v2の方向性を人間が確認する。
...
- [x] v2スライド化の対応表を追加する。
- [x] v2カード台紙仕様を追加する。
- [x] Day1当日レクチャー用の8枚構成、カード連動、画像生成promptを追加する。
- [x] Day2当日レクチャー用の8枚構成、価値の種カード連動、画像生成promptを追加する。
- [x] Day2の具体例として、地域で働く若手医師へのオンライン指導事業の1枚infographic promptを追加する。
- [ ] Day1当日レクチャー用8枚構成を人間が確認する。
- [ ] Day2当日レクチャー用8枚構成を人間が確認する。
- [ ] gamified v2の方向性を人間が確認する。
- [ ] 親向けgamified v2の方向性を人間が確認する。
- [ ] Day1スライド画像を1枚ずつ生成して候補に置く。
- [ ] Day2スライド画像を1枚ずつ生成して候補に置く。
...
- Day1当日レクチャー用のv2設計では、8枚構成にし、価値変化カードを選ぶ行動とワークシート記入をつなげた。
- Day2当日レクチャー用のv2設計では、8枚構成にし、価値の種カードを選ぶ行動、具体的な誰かを見る行動、次に聞く問いを作る行動をつなげた。
- Day2の医療事業例では、医師が少ない地域で働く若手医師を具体的な誰かとして見せ、相談できない不安からオンラインで相談できる安心への変化を扱った。
...
2. `slides/kids/day1_gamified_v2_lecture_prompt_packet.md` の8枚構成を確認してください。
3. `slides/kids/day2_gamified_v2_lecture_prompt_packet.md` の8枚構成を確認してください。
4. `infographics/parents_one_page_plan_infographic.md` と `infographics/parents_four_day_overview_infographic.md` の方向性を確認してください。
5. 確認後、画像を1枚ずつ生成して候補レビューへ進んでください。
